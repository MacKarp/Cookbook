...
pub fn on_facebook_login_button_clicked() {
    let facebook_client_id = ClientId::new(
        include_str!("../../../../../../../facebook_client_id")
            .trim()
            .to_string(),
    );
    let facebook_client_secret = ClientSecret::new(
        include_str!("../../../../../../../facebook_client_secret")
            .trim()
            .to_string(),
    );

    let auth_url = AuthUrl::new("https://www.facebook.com/v10.0/dialog/oauth?".to_string())
    ...
    for stream in listener.incoming() {
        if let Ok(mut stream) = stream {
            ...
            match token_response {
                Ok(token) => {
                    let x = facebook_oauth(token.access_token().secret());
                    match x {
                        Ok(()) => {
